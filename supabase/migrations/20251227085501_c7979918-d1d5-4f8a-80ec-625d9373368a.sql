-- 커뮤니티 펀드 적립을 100개 토큰 이상일 때만 작동하도록 수정
...
BEGIN
  -- 1. 현재 공급량 확인 (업데이트 전)
  SELECT total_supply INTO v_current_supply
  FROM fanz_tokens
  WHERE id = p_token_id;
  ...
  -- 5. 커뮤니티 펀드 적립 (100개 이상일 때만 작동)
  -- 구매 후 공급량이 100개 이상일 때만 적립
  IF (v_current_supply + p_amount) >= 100 THEN
