CREATE OR REPLACE FUNCTION public.check_and_increment_vote_count(user_id_param uuid, target_type_param text, target_id_param uuid)
 RETURNS TABLE(can_vote boolean, is_first_vote_today boolean, remaining_votes integer, max_votes integer, current_level integer, completion_rewarded boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 사용자 레벨 조회
  -- NOTE: RETURNS TABLE의 output column 이름(current_level)과 컬럼명이 겹치면 모호성(ambiguous) 에러가 발생하므로
  --       테이블 별칭으로 명확히 지정한다.
  SELECT p.current_level INTO user_level
  FROM profiles p
