...
contract KTrendzVote {
    ...
    event Vote(
        ...
        string artist,              // 아티스트명 (wiki entry title)
        string inviteCode,          // 초대코드 (추천인 추적)
    ...
    /**
     * @dev 투표 기록 - 플랫폼 백엔드에서 호출
     * @param voter 투표자 지갑 주소
     * @param artist 아티스트명
     * @param inviteCode 초대코드 (없으면 빈 문자열)
    ...
    function vote(
        ...
        emit Vote(voter, artist, inviteCode, voteCount, block.timestamp);
    }
